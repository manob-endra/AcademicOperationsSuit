...
function EditTeacherModal({ teacher, onClose, onSave }) {
  const [form, setForm] = useState({
    ...
    special_post: teacher.special_post || '',
    contact_number: teacher.contact_number || '',
    availability_status: teacher.availability_status || 'available',
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    setError('');
    const result = await onSave(teacher.id, {
      ...form,
      name: form.name.trim(),
    ...
    <div className="tm-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="tm-modal">
        <div className="tm-modal-header">
          <h2 className="tm-modal-title">Edit Teacher</h2>
