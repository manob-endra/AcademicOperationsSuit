...
export default function StudentAcademicCalendar() {
  ...
  const [publishedMap,    setPublishedMap]     = useState({}); // semesterId → calendarData
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Pre-select the first semester that has a published calendar
  ...
  const handleSemesterChange = async (semId) => {
    ...
    if (publishedMap[semId]) {
      setCalendarData(publishedMap[semId]);
      return;
    }

    // Not in published list — fetch directly (might be unpublished)
    setCalLoading(true);
  ...
  return (
    <div style={S.wrap}>
      <div style={S.hdr}>
        ...
        <p style={S.sub}>Published academic calendars for current and upcoming semesters.</p>
      </div>

      {!hasPublished && (
        <div style={S.empty}>
          No academic calendar has been published yet.<br />Check back later.
        </div>
      )}
