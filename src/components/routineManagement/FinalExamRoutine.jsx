...
const TH = { padding:'9px 12px', textAlign:'left', fontSize:11, fontWeight:700, border:'1px solid #5b21b6', whiteSpace:'nowrap' };
const TD = { padding:'8px 12px', border:'1px solid #e5e7eb', verticalAlign:'top' };

export default function FinalExamRoutine({ selectedSemesters = [] }) {
  ...
  const [semSessions, setSemSessions] = useState({});
  const [editSlots,   setEditSlots]   = useState({});
  const [invigState,  setInvigState]  = useState({});
  ...
  const loadSession = useCallback(async (semId) => {
    if (!semId) return;
    const res = await examRoutineAPI.getSession('final', semId);
    ...
    setConfigs(prev => ({
      ...prev, [semId]: prev[semId] || {
        startTime:      session.default_start_time    || '09:00',
        durationMins:   session.default_duration_mins || 180,
  ...
  const cfg     = s => configs[s] || { startTime:'09:00', durationMins:180, teachersPerExam:3, rooms:'' };
  ...
  const saveSlots = async semId => {
    ...
    setSaving(true);
    const c = cfg(semId);
    await examRoutineAPI.updateConfig('final', session.id, {
      teachers_per_exam: c.teachersPerExam,
      default_start_time: c.startTime,
      default_duration_mins: c.durationMins,
    });
    const res = await examRoutineAPI.saveSlots('final', session.id, slots.map((s, i) => ({
  ...
  const cycleWeight = async (semId, tid) => {
    ...
    const session = sess(semId);
    if (session?.id) await examRoutineAPI.setWeight('final', session.id, tid, next);
  };
  ...
  const autoAssign = async semId => {
    ...
    setSaving(true);
    const c = cfg(semId);
    const res = await examRoutineAPI.autoAssign('final', session.id, semId, wMap(semId), c.teachersPerExam);
    if (res.success) {
  ...
  const saveInvigilators = async semId => {
    ...
    if (!session?.id) return;
    setSaving(true);
    const res = await examRoutineAPI.saveInvigilators('final', session.id, invigState[semId] || {});
  ...
  const publish = async semId => {
    ...
    setPublishing(true);
    const res = await examRoutineAPI.publish('final', session.id);
    if (res.success) {
  ...
  return (
    <div>
      <div style={{ display:'flex', overflowX:'auto', borderBottom:'2px solid #e5e7eb', background:'#faf5ff', padding:'0 24px' }}>
        {sorted.map(sid => (
          <button key={sid} onClick={() => { setActiveSem(sid); setMsg({ type:'', text:'' }); setActiveTab('slots'); }}
            style={{
              ...
              borderBottom: sid === semId ? '2.5px solid #7c3aed' : '2.5px solid transparent',
              color: sid === semId ? '#7c3aed' : '#6b7280', marginBottom:-2, whiteSpace:'nowrap',
            }}>
            {SEM_LABEL[sid] || sid}
      ...
      <div style={{ padding:'20px 24px' }}>
        <div style={{ marginBottom:16 }}>
          <h3 style={{ fontSize:16, fontWeight:700, color:'#7c3aed', margin:'0 0 3px' }}>
            Final Exam Routine — {SEM_FULL[semId] || semId}
          </h3>
          ...
          <>
            <div style={{
              ...
              <label style={{ display:'flex', flexDirection:'column', gap:4 }}>
                ...
                <input type='number' min={30} max={480} value={c.durationMins}
                  onChange={e => updCfg(semId,'durationMins',parseInt(e.target.value)||180)}
              ...
              <label style={{ display:'flex', flexDirection:'column', gap:4, flex:1, minWidth:220 }}>
                <span style={{ fontWeight:600, color:'#6b7280', fontSize:11 }}>Default Exam Rooms (e.g. 429, 430)</span>
            ...
            <div style={{ display:'flex', borderBottom:'2px solid #e5e7eb', marginBottom:20 }}>
              {[['slots','Exam Slots'],['weights','Invigilator Weights'],['assign','Assign Invigilators']].map(([k,lbl]) => (
                <button key={k} onClick={() => setActiveTab(k)} style={{
                  ...
                  borderBottom: activeTab===k ? '2.5px solid #7c3aed' : '2.5px solid transparent',
                  color: activeTab===k ? '#7c3aed' : '#6b7280', marginBottom:-2,
                }}>{lbl}</button>
              ))}
            </div>
              ...
              <div>
                              ...
                              <td style={TD}>
                                ...
                                {!slot.rooms && c.rooms && (
                                  <button onClick={() => updEditSlot(semId, idx, 'rooms', c.rooms)}
                                    style={{ display:'block', marginTop:3, fontSize:10, cursor:'pointer', color:'#7c3aed', background:'none', border:'none', padding:0, textDecoration:'underline' }}>
                                    Use default ({c.rooms})
                                  </button>
                                )}
                              </td>
                ...
                {slots.length > 0 && (
                  <button onClick={() => saveSlots(semId)} disabled={saving}
                    style={{ padding:'9px 22px', borderRadius:8, border:'none', background:'#7c3aed', color:'white', fontSize:13, fontWeight:700, cursor:'pointer', opacity:saving?0.7:1 }}>
            ...
            {activeTab === 'weights' && (
              <div>
                <p style={{ fontSize:13, color:'#6b7280', marginBottom:16 }}>
                  Click to cycle: 0 = Excluded, 1 = Own course only, 2 = Full duty.
                  Chairman / Dean default to 1. Teachers on leave are auto-skipped.
                </p>
                  ...
                  {allTeachers.map(t => {
                    ...
                    return (
                      <button key={t.id} onClick={() => cycleWeight(semId, t.id)}
                        ...
                        <span style={{ width:34, height:34, borderRadius:'50%', flexShrink:0, background: onLeave?'#dc2626':'#7c3aed', color:'white', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700 }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          {onLeave && <div style={{ fontSize:10, color:'#dc2626', fontWeight:600, marginTop:2 }}>On leave · {lDates.length} exam date{lDates.length>1?'s':''}</div>}
                        </div>
                        {onLeave ? (
                      ...
                      return (
                        <div key={sIdx} style={{ border:'1.5px solid #e5e7eb', borderRadius:10, padding:'14px 16px', background:'white' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10, flexWrap:'wrap' }}>
                            <span style={{ fontWeight:700, color:'white', fontSize:12, background:'#7c3aed', borderRadius:4, padding:'2px 8px', flexShrink:0 }}>{sIdx+1}</span>
                            ...
                            {allTeachers.map(t => {
                              ...
                              return (
                                <button key={t.id}
                                  ...
                                  title={onLeave ? `${t.name} is on leave on ${slot.exam_date}` : t.name}
                                  style={{
                                    padding:'4px 10px', borderRadius:20, fontSize:11, fontWeight:600,
                                    cursor: onLeave ? 'not-allowed' : 'pointer', border:'1.5px solid',
                                    borderColor: onLeave ? '#fca5a5' : isOn ? '#7c3aed' : '#e5e7eb',
                                    background:  onLeave ? '#fef2f2' : isOn ? '#7c3aed' : 'white',
                                    color:        onLeave ? '#dc2626' : isOn ? 'white' : '#6b7280',
                                    textDecoration: onLeave ? 'line-through' : 'none', opacity: onLeave ? 0.65 : 1,
                                  }}>
                          ...
                          {invigs.length > 0 && (
                            <div style={{ paddingTop:8, borderTop:'1px solid #f1f5f9' }}>
                              <div style={{ fontSize:10, fontWeight:600, color:'#9ca3af', textTransform:'uppercase', marginBottom:4 }}>Assigned Invigilators</div>
                              {invigs.map((inv, i) => {
            ...
            <div style={{ marginTop:24, paddingTop:20, borderTop:'1.5px solid #e5e7eb', display:'flex', alignItems:'center', gap:14 }}>
              <button onClick={() => publish(semId)} disabled={publishing || !saved.length}
                style={{ padding:'10px 28px', borderRadius:8, border:'none', background: session?.published ? '#374151' : '#6d28d9', color:'white', fontSize:14, fontWeight:700, cursor:'pointer', opacity:publishing?0.7:1 }}>
                {publishing ? 'Publishing…' : session?.published ? 'Re-publish' : 'Publish final exam routine'}
              </button>
              {session?.published && (
                <span style={{ fontSize:12, color:'#6b7280' }}>Last published {new Date(session.published_at).toLocaleString()}</span>
              )}
            </div>
          </>
        )}
