...
const generateTimeSlots = (
  ...
  const skipMinutes = parseInt(skipTime) ?? 5; // Use nullish coalescing to handle 0 correctly
...
function TimePickerModal({ isOpen, title, currentTime, onConfirm, onCancel, showAmPm = false }) {
  ...
  // Calculate period from hours if showAmPm
  ...
  const handleConfirm = () => {
    ...
    // Always store just HH:MM format, never include AM/PM in storage
    onConfirm(timeValue);
  };
...
function TimeSlot() {
  ...
  const [classTimeDetails, setClassTimeDetails] = useState(initialClassTimeDetails); // User's current edits
  const [appliedClassTimeDetails, setAppliedClassTimeDetails] = useState(initialClassTimeDetails); // What grid uses
  const [saveStatus, setSaveStatus] = useState('saved'); // saved or pending
  ...
  // Save All handler - applies all pending changes to the grid
  const handleSaveAll = () => {
    setAppliedClassTimeDetails(classTimeDetails);
    setSaveStatus('saved');
  };

  // Generate time slots and working days dynamically based on APPLIED settings
  ...
  return (
    <div className="routine-section-content timeslot-container">
      ...
      <div className="timeslot-tabs">
        <button
          className={`tab-btn ${activeTab === 'classTimeDetails' ? 'active' : ''}`}
          onClick={() => setActiveTab('classTimeDetails')}
        >
          Class Time Details
        </button>
        <button
          className={`tab-btn ${activeTab === 'classroomClusters' ? 'active' : ''}`}
          onClick={() => setActiveTab('classroomClusters')}
        >
          Classroom Clusters
        </button>
        <button
          className={`tab-btn ${activeTab === 'courseTime' ? 'active' : ''}`}
          onClick={() => setActiveTab('courseTime')}
        >
          Course Time
        </button>
        <button
          className={`tab-btn ${activeTab === 'teachersTime' ? 'active' : ''}`}
          onClick={() => setActiveTab('teachersTime')}
        >
          Teacher's Time
        </button>
      </div>
      ...
      {activeTab === 'classroomClusters' && (
        <div className="tab-content">
          <p>Classroom Clusters configuration will be implemented here.</p>
        </div>
      )}

      {activeTab === 'courseTime' && (
        <div className="tab-content">
          <p>Course Time configuration will be implemented here.</p>
        </div>
      )}

      {activeTab === 'teachersTime' && (
        <div className="tab-content">
          <p>Teacher's Time configuration will be implemented here.</p>
        </div>
      )}
